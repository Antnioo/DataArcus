...
const TOOLS = [
  ...
  ['svg-kpi-designer', 'svg-kpi-designer', 'SVG KPI Designer for Power BI', ['Link to measures', 'Sparklines', 'One DAX measure'], 'tool-svgkpi.jpg']
];
...
const HUB = [
  ...
  ['SVG KPI designer', 'KPI cards as DAX', icon('<rect x="3" y="5" width="18" height="14" rx="2"/><path d="M7 15l3-3 3 2 4-5"/>')]
];
...
const JOBS = [
  ...
  { id: 'home', html: page(textBlock({ kicker: 'Power BI consulting · Dubai', title: 'From Scattered Business Data to Decisions You Can Trust', size: 42, chips: ['8 showcase dashboards', '8 free tools', 'English & Arabic'] }),
  ...
  { id: 'tools', html: page(textBlock({ kicker: 'Free · No sign-up · English & Arabic', title: 'Free Power BI & Fabric Tools', sub: 'Build, check and learn. Nothing to install.' }), (() => {
    // the hub's three groups; HUB order is DP-600, health, licensing, theme, calendar, measures, PL-300, SVG KPI designer
    ...
    return `<div style="display:flex;flex-direction:column;gap:16px">${group('Build', [HUB[7], HUB[3], HUB[4], HUB[5]], 2)}${group('Check', [HUB[1], HUB[2]], 2)}${group('Learn', [HUB[0], HUB[6]], 2)}</div>`;
  })(), '430px 1fr') },
