...
window.journeyAttributionTranslations = {
  en: {
    ...
    meta: {
      ...
      "og:image": "https://dataarcus.com/assets/img/og/journey-attribution.jpg",
      ...
      "twitter:image": "https://dataarcus.com/assets/img/og/journey-attribution.jpg",
    ...
    // Article Content
    content: {
      p1: "Ask three different reports \"how many leads did Campaign X actually convert?\" and you'll often get three different numbers. Nobody lied. Each report matched the same lead to a call, an outcome, and an invoice using slightly different logic.",
      ...
      p_sec1_1: "In a dealership CRM, a single lead can touch dozens of records: multiple calls, several statuses, and maybe, eventually, an invoice. The natural instinct is to let every report answer its own question independently: the Sales report looks up the invoice, the Call report looks up the last call, the Qualification report looks up the latest status.",
      ...
      p_sec2_3: "That single validation pass is what makes the pattern trustworthy, for us and for anyone reading the model later. One resolved key, proven correct once, is worth more than a dozen independently \"probably correct\" joins.",
      ...
      p_sec3_3: "The result in DataArcus Pulse: 54,574 leads, 113,321 calls, and 72,013 call journeys that agree with each other on every page, from Executive Overview to Sales &amp; Attribution, because they all trace back to the same resolved key."
    },
  ...
  ar: {
    ...
    // Article Content
    content: {
      p1: "اسأل ثلاثة تقارير مختلفة \"كم عدد العملاء المحتملين الذين حوّلتهم الحملة X فعليًا؟\" وستحصل غالبًا على ثلاثة أرقام مختلفة. لم يكذب أحد. كل تقرير طابق نفس العميل المحتمل بمكالمة ونتيجة وفاتورة باستخدام منطق مختلف قليلاً.",
      p2: "هذه هي <strong>فجوة العزو</strong>: يُعاد حل رحلة العميل نفسها بشكل مستقل في كل مرة يحتاج فيها مقياس جديد لمعرفة \"ماذا حدث لهذا العميل المحتمل\". الفروقات الصغيرة في منطق المطابقة، مثل نافذة تاريخ مختلفة أو قاعدة ترجيح مختلفة، تتراكم لتنتج مؤشرات أداء تتعارض مع بعضها بصمت.",
      ...
      p_sec1_1: "في نظام CRM لوكالة سيارات، يمكن لعميل محتمل واحد أن يلامس عشرات السجلات: مكالمات متعددة، عدة حالات، وربما في النهاية فاتورة. الغريزة الطبيعية هي ترك كل تقرير يجيب على سؤاله بشكل مستقل: تقرير المبيعات يبحث عن الفاتورة، تقرير المكالمات يبحث عن آخر مكالمة، تقرير التأهيل يبحث عن أحدث حالة.",
      ...
      p_sec2_3: "عملية التحقق الواحدة هذه هي ما يجعل النمط موثوقًا، لنا ولأي شخص يقرأ النموذج لاحقًا. مفتاح واحد تم حله وإثبات صحته مرة واحدة يساوي أكثر من عشرات الروابط المستقلة \"الصحيحة على الأرجح\".",
      ...
      p_sec3_3: "النتيجة في داتا أركوس بلس: 54,574 عميلاً محتملاً، و113,321 مكالمة، و72,013 رحلة اتصال تتفق جميعها مع بعضها في كل صفحة، من النظرة التنفيذية العامة إلى المبيعات والعزو، لأنها جميعًا تعود إلى نفس المفتاح المحلول."
    },
