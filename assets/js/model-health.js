...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function tMeasures() {
    ...
    const chip = (d) => '<button type="button" class="mh-dep ' + d.type + '" ' + (d.type === 'measure' ? 'data-ms="' + esc(d.name) + '"' : '') + '><i class="bi ' + (d.type === 'measure' ? 'bi-calculator' : d.type === 'column' ? 'bi-layout-three-columns' : 'bi-table') + '"></i> ' + (d.type === 'column' ? esc(d.table) + '[' + esc(d.name) + ']' : d.type === 'measure' ? '[' + esc(d.name) + ']' : esc(d.name)) + '</button>';
    el.innerHTML = '<div class="row g-4"><div class="col-lg-5"><div class="mh-panel mh-mlist"><div class="mh-search"><i class="bi bi-search"></i><input type="search" id="mhQ" value="' + esc(msQuery) + '" placeholder="' + L('Search name, folder or DAX', 'ابحث بالاسم أو المجلد أو DAX') + '"></div>' +
      ...
      '<div class="col-lg-7"><div class="mh-panel mh-mdetail">' + (sel ?
        ...
        '<div class="mh-meta">' + (sel.folder ? '<span><i class="bi bi-folder2"></i> ' + bdi(sel.folder) + '</span>' : '') + '<span><i class="bi bi-table"></i> ' + bdi(sel.table) + '</span>' + (sel.formatString ? '<span><i class="bi bi-hash"></i> ' + bdi(sel.formatString) + '</span>' : '') + '<span><i class="bi bi-bar-chart"></i> ' + L('in ' + num(sel.visuals) + ' visual refs', 'في ' + num(sel.visuals) + ' مرجع visual') + '</span><span><i class="bi bi-layers"></i> ' + L('chain depth ' + sel.depth, 'عمق السلسلة ' + sel.depth) + '</span></div>' +
  ...
  function tDocs() {
    ...
    el.querySelectorAll('[data-doc]').forEach((b) => b.onclick = () => {
      ...
      if (b.dataset.doc === 'csv') {
        const rows = [['Measure', 'Table', 'Folder', 'Format', 'Used', 'Visual refs', 'Depth', 'Description', 'DAX']].concat(R.measures.map((m) => [m.name, m.table, m.folder, m.formatString, m.used == null ? '' : m.used ? 'yes' : 'no', m.visuals, m.depth, m.description, m.expr.trim()]));
  ...
  function docHtml() {
    ...
    return '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>' + h(R.meta.fileName) + ' - model documentation</title><style>' +
      ...
      '<h2>Measures</h2>' + Object.keys(folders).sort().map((k) => '<h3>' + h(k) + ' (' + folders[k].length + ')</h3>' + folders[k].map((m) => '<div class="m"><b>[' + h(m.name) + ']</b> <span class="meta">' + (m.formatString ? 'Format ' + h(m.formatString) + ' · ' : '') + (m.used == null ? '' : m.used ? 'used in ' + m.visuals + ' visual refs' : '<span class="no">unused</span>') + '</span>' + (m.description ? '<div>' + h(m.description) + '</div>' : '') + '<pre>' + h(m.expr.trim()) + '</pre>' + (m.dependsOn.length ? '<div class="meta">Depends on: ' + m.dependsOn.map((x) => x.type === 'measure' ? '[' + h(x.name) + ']' : x.type === 'column' ? h(x.table) + '[' + h(x.name) + ']' : h(x.name)).join(', ') + '</div>' : '') + '</div>').join('')).join('') +
  ...
  function docMd() {
    ...
    R.measures.slice().sort((a, b) => (a.folder || '').localeCompare(b.folder || '') || a.name.localeCompare(b.name)).forEach((m) => {
      lines.push('### ' + m.name, '', (m.folder ? 'Folder: ' + m.folder + '  ' : '') + (m.formatString ? 'Format: `' + m.formatString + '`  ' : '') + (m.used == null ? '' : m.used ? 'Used in ' + m.visuals + ' visual refs' : 'Unused'), '');
